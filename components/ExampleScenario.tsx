  ...
  return (
    <div className="card space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs font-bold uppercase tracking-wide rounded-full bg-accent/10 text-accent px-2.5 py-1">
          Örnek senaryo
        </span>
      ...
      <ol className="space-y-3">
        {steps.map((s, i) => (
          <li key={i} className="flex gap-3 text-sm">
            <span className="shrink-0 mt-0.5 w-5 h-5 rounded-full bg-muted text-ink/50 text-xs font-bold flex items-center justify-center">
              {i + 1}
            </span>
            <div>
              <div className="font-semibold text-ink/80">{s.title}</div>
